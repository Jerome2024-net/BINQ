...
export default function EvenementPage() {
  ...
  function handleInviteFriends() {
    ...
    if (navigator.share) {
      navigator.share({ title: event.nom, text: shareText, url: shareUrl }).catch(() => {});
    } else {
  ...
  if (success && ticketCodes.length > 0) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6" style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
        ...
        <p className="text-gray-400 text-[14px] mb-8 text-center">
          {ticketCodes.length > 1 ? `${ticketCodes.length} billets` : "1 billet"} pour{" "}
          <span className="text-gray-900 font-medium">{event.nom}</span>
        </p>

        <div className="w-full max-w-sm space-y-2.5 mb-6">
          {ticketCodes.map((code, i) => (
            <a key={code} href={`/billet/${code}`} className="flex items-center justify-between w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-sm font-medium text-gray-900 hover:bg-gray-100 transition">
              <span className="flex items-center gap-2">
                <Ticket className="w-4 h-4 text-gray-400" />
                {ticketCodes.length > 1 ? `Billet ${i + 1}` : "Voir mon billet"}
              </span>
        ...
        {buyerPhone.trim() && (
          <div className="w-full max-w-sm mb-6">
            <button
              onClick={() => {
                const phone = buyerPhone.trim().replace(/\s+/g, "").replace(/^0/, "225");
                const baseUrl = typeof window !== "undefined" ? window.location.origin : "";
                const billetLinks = ticketCodes.map((code, i) =>
                  ticketCodes.length > 1 ? `Billet ${i + 1}: ${baseUrl}/billet/${code}` : `${baseUrl}/billet/${code}`
                ).join("\n");
                const msg = encodeURIComponent(
                  `🎟 Votre billet pour *${event.nom}*\n\n` +
                  `📅 ${fmtDate(event.date_debut)}${event.heure_debut ? ` à ${fmtTime(event.heure_debut)}` : ""}\n` +
                  `📍 ${event.lieu}${event.ville ? `, ${event.ville}` : ""}\n\n` +
                  `${billetLinks}\n\n` +
                  `Présentez le QR code à l'entrée. Bonne soirée ! 🎉`
                );
                window.open(`https://wa.me/${phone.startsWith("+") ? phone.slice(1) : phone}?text=${msg}`, "_blank");
              }}
              className="w-full py-3 bg-[#25D366] text-white rounded-xl text-sm font-medium flex items-center justify-center gap-2 hover:bg-[#20BD5A] transition"
            >
        ...
        {totalParticipants > 0 && (
          <p className="mt-5 text-[12px] text-gray-400">
            <span className="font-semibold text-gray-500">{totalParticipants}</span> personne{totalParticipants > 1 ? "s" : ""} inscrite{totalParticipants > 1 ? "s" : ""}
          </p>
        )}
