  ...
  const handleDefaillance = (membreId: string, membreNom: string) => {
    setConfirmModal({
      ...
      action: async () => {
        ...
        if (result.success) {
          // Les remboursements seront traités automatiquement via Stripe
          showToast(
            "success",
            "Défaillance signalée",
            result.defaillantNom + " a été exclu. Tontine annulée. " + (result.membresRembourses || 0) + " membres seront remboursés via Stripe."
          );
        } else {
