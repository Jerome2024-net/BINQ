...
APP_NAME="Binq Livreur"
BUNDLE_ID="io.binq.livreur"
ORG="io.binq"
...
if [ -f "ios/Runner/Info.plist" ]; then
  ...
  /usr/libexec/PlistBuddy -c "Add :NSLocationWhenInUseUsageDescription string Binq utilise votre position pour vous guider vers le client." ios/Runner/Info.plist 2>/dev/null \
    || /usr/libexec/PlistBuddy -c "Set :NSLocationWhenInUseUsageDescription Binq utilise votre position pour vous guider vers le client." ios/Runner/Info.plist
fi
